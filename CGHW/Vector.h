...
public:
    ...
    inline Vector3 Map(const float wxl, const float wxr, const float wyb, const float wyt, const float vxl, const float vxr, const float vyb, const float vyt) const {
        return Vector3((x - wxl) * (vxr - vxl) / (wxr - wxl) + vxl, (y - wyb) * (vyt - vyb) / (wyt - wyb) + vyb, 1);
    }
...
public:
    ...
    inline Vector4 Map(const float wxl, const float wxr, const float wyb, const float wyt, const float vxl, const float vxr, const float vyb, const float vyt) const {
        return Vector4((x - wxl) * (vxr - vxl) / (wxr - wxl) + vxl, (y - wyb) * (vyt - vyb) / (wyt - wyb) + vyb, z, 1);
    }
...
public:
    ...
    inline void MakeGRM(const Vector3& r, const Vector3& u, const Vector3& f) {
        m[0] = r.x; m[1] = r.y; m[2] = r.z; m[3] = 0.0f;
        m[4] = u.x; m[5] = u.y; m[6] = u.z; m[7] = 0.0f;
        m[8] = f.x; m[9] = f.y; m[10] = f.z; m[11] = 0.0f;
        m[12] = 0.0f; m[13] = 0.0f; m[14] = 0.0f; m[15] = 1.0f;
    }
    inline void MakePM(float f, float h, float y) {
        m[0] = 1.0f; m[1] = 0.0f; m[2] = 0.0f; m[3] = 0.0f;
        m[4] = 0.0f; m[5] = 1.0f; m[6] = 0.0f; m[7] = 0.0f;
        m[8] = 0.0f; m[9] = 0.0f; m[10] = y*tan(f)/(y-h); m[11] = h*y*tan(f)/(h-y);
        m[12] = 0.0f; m[13] = 0.0f; m[14] = tan(f); m[15] = 0.0f;
    }
    ...
    inline static Matrix4 GRM(const Vector3& r, const Vector3& u, const Vector3& f) { Matrix4 m; m.MakeGRM(r, u, f); return m; }
    inline static Matrix4 PM(float f, float h, float y) { Matrix4 m; m.MakePM(f, h, y); return m; }
...
public:
    //Constructors
    TrianglesMatrix() {}
    explicit TrianglesMatrix(float b) { Fill(b); }
    ...
    inline void Make(const Vector3& v1, const Vector3& v2, const Vector3& v3) {
        m[0] = v1.x; m[1] = v2.x; m[2] = v3.x;
        m[3] = v1.y; m[4] = v2.y; m[5] = v3.y;
        m[6] = v1.z; m[7] = v2.z; m[8] = v3.z;
        m[9] = 1.0f; m[10] = 1.0f; m[11] = 1.0f;
    }
    inline void MakePD() {
        m[0] = m[0] / m[9]; m[1] = m[1] / m[10]; m[2] = m[2] / m[11];
        m[3] = m[3] / m[9]; m[4] = m[4] / m[10]; m[5] = m[5] / m[11];
        m[6] = m[6] / m[9]; m[7] = m[7] / m[10]; m[8] = m[8] / m[11];
        m[9] = 1.0f; m[10] = 1.0f; m[11] = 1.0f;
    }
    inline void MakeView(const float wxl, const float wxr, const float wyb, const float wyt, const float vxl, const float vxr, const float vyb, const float vyt) {
        m[0] = (m[0] - wxl) * (vxr - vxl) / (wxr - wxl) + vxl; m[1] = (m[1] - wxl) * (vxr - vxl) / (wxr - wxl) + vxl; m[2] = (m[2] - wxl) * (vxr - vxl) / (wxr - wxl) + vxl;
        m[3] = (m[3] - wyb) * (vyt - vyb) / (wyt - wyb) + vyb; m[4] = (m[4] - wyb) * (vyt - vyb) / (wyt - wyb) + vyb; m[5] = (m[5] - wyb) * (vyt - vyb) / (wyt - wyb) + vyb;
        m[6] = m[6]; m[7] = m[7]; m[8] = m[8];
        m[9] = m[9]; m[10] = m[10]; m[11] = m[11];
    }

    //Statics
    inline static TrianglesMatrix Zero() { TrianglesMatrix m; m.MakeZero(); return m; }
    inline static TrianglesMatrix Triangle(const Vector3& v1, const Vector3& v2, const Vector3& v3) { TrianglesMatrix m; m.Make(v1, v2, v3); return m; }
    inline static TrianglesMatrix PD() { TrianglesMatrix m;  
    //std::cout << "test PD" << std::endl; std::cout << m[0] << ", " << m[1] << ", " << m[2] << std::endl; 
    m.MakePD(); return m; }
    inline static TrianglesMatrix View(const float wxl, const float wxr, const float wyb, const float wyt, const float vxl, const float vxr, const float vyb, const float vyt) { 
        TrianglesMatrix m; 
        //std::cout << "test V" << std::endl; std::cout << m[0] << ", " << m[1] << ", " << m[2] << std::endl; 
        m.MakeView(wxl, wxr, wyb, wyt, vxl, vxr, vyb, vyt); return m;
    }

    //Some getters
    inline Vector3 XAxis() const {
        return Vector3(m[0], m[3], m[6]);
    }
    inline Vector3 YAxis() const {
        return Vector3(m[1], m[4], m[7]);
    }
    inline Vector3 ZAxis() const {
        return Vector3(m[2], m[5], m[8]);
    }
    inline Vector3 getw() const {
        return Vector3(m[9], m[10], m[11]);
    }
    inline float operator[](const int i) const {
        return m[i];
    }
    inline Vector4 point1() const {
        return Vector4(m[0], m[3], m[6], m[9]);
    }
    inline Vector4 point2() const {
        return Vector4(m[1], m[4], m[7], m[10]);
    }
    inline Vector4 point3() const {
        return Vector4(m[2], m[5], m[8], m[11]);
    }
    ...
    inline void operator*=(float b) {
        for (int i = 0; i < 12; ++i) { m[i] *= b; }
    }

    //Multiplication
    TrianglesMatrix operator*(const Matrix4& b) const {
        TrianglesMatrix out;

        out.m[0] = b.m[0] * m[0] + b.m[1] * m[3] + b.m[2] * m[6] + b.m[3] * m[9];
        out.m[1] = b.m[0] * m[1] + b.m[1] * m[4] + b.m[2] * m[7] + b.m[3] * m[10];
        out.m[2] = b.m[0] * m[2] + b.m[1] * m[5] + b.m[2] * m[8] + b.m[3] * m[11];

        out.m[3] = b.m[4] * m[0] + b.m[5] * m[3] + b.m[6] * m[6] + b.m[7] * m[9];
        out.m[4] = b.m[4] * m[1] + b.m[5] * m[4] + b.m[6] * m[7] + b.m[7] * m[10];
        out.m[5] = b.m[4] * m[2] + b.m[5] * m[5] + b.m[6] * m[8] + b.m[7] * m[11];

        out.m[6] = b.m[8] * m[0] + b.m[9] * m[3] + b.m[10] * m[6] + b.m[11] * m[9];
        out.m[7] = b.m[8] * m[1] + b.m[9] * m[4] + b.m[10] * m[7] + b.m[11] * m[10];
        out.m[8] = b.m[8] * m[2] + b.m[9] * m[5] + b.m[10] * m[8] + b.m[11] * m[11];

        out.m[9] = b.m[12] * m[0] + b.m[13] * m[3] + b.m[14] * m[6] + b.m[15] * m[9];
        out.m[10] = b.m[12] * m[1] + b.m[13] * m[4] + b.m[14] * m[7] + b.m[15] * m[10];
        out.m[11] = b.m[12] * m[2] + b.m[13] * m[5] + b.m[14] * m[8] + b.m[15] * m[11];
